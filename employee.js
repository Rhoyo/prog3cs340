module.exports = function(){
    ...
    function getEmployeebyProject(req, res, mysql, context, complete){
	  ...
	  var query2 = "SELECT  Pname, Plocation FROM PROJECT WHERE Pnumber = ?";
	  var inserts2 = [req.params.project];
	  mysql.pool.query(query2, inserts2, function(error, results2, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
    ...
    /* Find employee whose fname starts with a given string in the req */
    function getEmployeeWithNameLike(req, res, mysql, context, complete) {
      //sanitize the input as well as include the % character
       var query = "SELECT Fname, Lname, Dno FROM EMPLOYEE WHERE EMPLOYEE.Fname LIKE " + mysql.pool.escape(req.params.s + '%');
      console.log(query)
    ...
    /*Display all employees from a given project. Requires web based javascript to delete users with AJAX*/
